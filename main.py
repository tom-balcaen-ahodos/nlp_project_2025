...
file_name = "train_set_md_df_filtered_200k.csv"
def main():
    df = pd.read_csv('../salesforce_case_v2.csv', sep=';', dtype=str, on_bad_lines='skip', skiprows=range(1, 1), nrows=200000) #, nrows=10000
    ...
    print(df_filtered['Priority'].unique())
    print(df_filtered['AGR_Type_of_Complaint__c'].unique())
    ...
    # Remove rows with values in 'AGR_Type_of_Complaint__c'
    ...
    small_df_filtered = df_filtered[['Subject','TextBody','Complaint','Complaint_type']].copy()
    small_df_complaints = df_filtered_complaints[['Subject','TextBody','Complaint','Complaint_type']].copy()
    small_df_no_complaints = df_filtered_no_complaints[['Subject','TextBody','Complaint','Complaint_type']].copy()
    
    #only take 1000 of each
    ...
    save_to_csv(small_df_filtered, file_name) #output_small_df_filtered.csv
...
def save_to_csv(df, output_file, record_limit=number_of_rows):
...
def remove_unwanted_rows(df):
    # Remove rows containing 'Automatic reply' or 'Automatisch antwoord'
    mask = (
        # ~df['Name'].str.contains('Automatic reply|Automatisch antwoord|Fw:|RE:', case=False, na=False) &
        ~df['Subject'].str.contains('Automatic reply|Automatisch antwoord|Fw:|RE:', case=False, na=False)
        # ~df['TextBody'].str.contains('Automatic reply|Automatisch antwoord|Fw:|RE:', case=False, na=False)
    )
    df_filtered = df[mask]
    return df_filtered
